...
earnings_hist<- ggplot(data = cps, 
  ...
  labs(x = "Salario por hora trabajada (dólares)", y = "Porcentaje") +
  facet_wrap(~ifelse(female, "Mujer", "Hombre"))+
  labs(x = "Salario por hora trabajada (dólares)",y = "Porcentaje")+
...
ggplot(cps, aes(x="w", y="2*stat(count)/sum(stat(count))")) + geom_histogram(
    ...
    show_legend=False,
    na_rm=True,
) + labs(x="Salario por hora trabajada (dólares)", y="Porcentaje") + facet_wrap('~female',labeller={'0':"Hombre",'1':"Mujer"}
) + labs(
    x="Salario por hora trabajada (dólares)", y="Porcentaje"
) + scale_x_continuous(
    limits=(0, 100),
) + scale_y_continuous(
