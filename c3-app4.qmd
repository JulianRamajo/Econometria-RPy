...
::: justify
# Aplicación 3.4: Heteroscedasticidad {.unnumbered}

## Función de demanda con varianza no constante {.unnumbered}

Empleando datos de corte transversal, en este ejemplo se va a estimar un modelo de demanda para un determinado producto A:
...
Una vez chequeada la hipótesis de varianza constante (homoscedasticidad), se plantea la estimación por mínimos cuadrados ponderados (MCP) de la función de demanda bajo la hipótesis de un modelo heteroscedástico asociad al logaritmo de la renta del tipo siguiente:

$${σ_i}^2 = e^{α_0 + α_1 log⁡(Y_i)}$$

Esta especificación para la varianza residual equivale a suponer que ${σ_i}^2 = σ^2 (Y_i)^{α_1}$, donde $σ^2 = e^{α_0}$, y de ahí que este tipo de especificación reciba el nombre de *heteroscedasticidad multiplicativa*.

De forma complementaria, compararemos los resultados obtenidos por MCP con los que se consiguen al estimar la función de demanda por MCO, pero corrigiendo la matriz de covarianzas usando el método de White (MCO-HC).
:::
...
# Corrección básica de la heteroscedasticidad: matriz de covarianzas robusta
